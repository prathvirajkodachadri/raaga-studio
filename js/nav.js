...
(function () {
  ...
  function sitePath(file) {
    ...
    var marker = '/raaga-studio/';
    var idx = base.indexOf(marker);
    var root = idx >= 0 ? base.slice(0, idx + marker.length) : './';
    return root + file;
  }
  ...
  function addExternalCalculatorLinks() {
    ...
    if (tabnav) {
      if (!tabnav.querySelector('a[data-tool="audio-calculators"]')) {
        ...
        a.className = 'tab'; a.href = sitePath('audio-calculators.html'); a.setAttribute('data-tool', 'audio-calculators'); a.setAttribute('aria-label', 'Audio Calculators');
      ...
      if (!tabnav.querySelector('a[data-tool="key-chords"]')) {
        ...
        k.className = 'tab'; k.href = sitePath('key-chords.html'); k.setAttribute('data-tool', 'key-chords'); k.setAttribute('aria-label', 'Key & Chords');
    ...
    if (grid) {
      if (!grid.querySelector('a[data-tool="audio-calculators"]')) {
        var d = document.createElement('a'); d.className = 'nav-drawer-item'; d.href = sitePath('audio-calculators.html'); d.setAttribute('data-tool', 'audio-calculators');
      ...
      if (!grid.querySelector('a[data-tool="key-chords"]')) {
        var kd = document.createElement('a'); kd.className = 'nav-drawer-item'; kd.href = sitePath('key-chords.html'); kd.setAttribute('data-tool', 'key-chords');
